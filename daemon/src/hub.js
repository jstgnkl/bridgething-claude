// WS hub at /ws. Clients: the device app, the emulator, the webpage.
// First frame must be bridge.hello {role}. Responses go ONLY to the requesting
// socket (avoids request-id collisions between clients); events broadcast to all.
...
export function createHub({ onHello } = {}) {
  ...
  // A client on a slow link (the SSH tunnel to the device) that stops draining
  ...
  wss.on('connection', (socket) => {
    ...
    socket.on('message', async (raw) => {
      ...
      if (msg.method === 'bridge.hello') {
        ...
        // whatever a screen has been showing since the old one died — the
        // device reconnects on its own, but it has no way to know which of its
        // tiles went stale while the socket was down.
