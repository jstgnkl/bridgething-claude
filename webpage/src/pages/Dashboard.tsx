...
const TOPICS = ['bridge.clients', 'claude.permission.request', 'claude.permission.resolved'];
...
export function Dashboard() {
  ...
  const emulatorOnline = (status?.clients?.emulator ?? 0) > 0;
  const deviceAppOnline = (status?.clients?.device ?? 0) > 0;
  ...
  return (
    <div>
      ...
      <Card className="mb-4">
        ...
        <StatusRow label="Device app" value={deviceAppOnline ? 'connected' : 'not connected'}
          tone={deviceAppOnline ? 'ok' : 'off'} hint="the Car Thing kiosk, over the reverse SSH tunnel" />
        <StatusRow label="Emulator" value={emulatorOnline ? 'attached' : 'not running'}
          tone={emulatorOnline ? 'ok' : 'off'} hint="local dev device" />
        <StatusRow label="Device reachable" value={deviceAppOnline || emulatorOnline ? 'yes' : 'no'}
          tone={deviceAppOnline || emulatorOnline ? 'ok' : 'bad'} />
      </Card>
