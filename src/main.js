...
var INPUT_ACTIONS = ['dial', 'select', 'back', 'deny', 'tap',
  'page-sessions', 'page-queue', 'page-usage', 'ambient'];
...
// what clears cards left behind by a daemon that restarted: the asks on screen
// belong to a dead process and no per-ask event will ever retract them, so the
// full list is the only thing that can say which ones are gone.
...
// The socket IS the daemon here, so its close is the disconnect and the
// reconnect's onOpen (which already re-syncs the queue) is the recovery — no
// synthesized link event needed.
ws.onClose(function () {
...
ws.onOpen(function () {
  // The open socket IS the link, so opening it is what proves the daemon is
  // there. Keying this to the sessions.list response instead would make one RPC
  // the sole writer of `true`, and a single failed call would strand the
  // OFFLINE banner on screen for good while snapshot events kept the tiles
...
// ws.js no longer connects on import — every listener above is registered by
// now, so opening the socket here is what makes them live.
ws.start();
