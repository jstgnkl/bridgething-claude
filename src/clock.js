...
// Date.now().
//
// The frame-level server_timestamp_ms is deliberately NOT used: a relay hop
// re-emits events with its own (device) clock, so on hardware that field can
// carry the very time we are correcting. The payload fields cross every path
// intact, so they are the ones read.
