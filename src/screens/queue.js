...
// The hero body is a fixed-width column of monospace, so how many lines a
// command takes is a property of its length: 732px of card at 22px JetBrains
// Mono is 55 columns, and word-break: break-all fills every one of them. A
// command that wraps costs the hero the same room an intent line does, so it
// is funded the same way — the stack gives a row back. styles.css holds the
// guarantee even where this estimate does not (a CJK path, a changed font):
// the summary is the only child of the hero body that may give way, so the
// chips and the header never move no matter what a command does.
var SUMMARY_COLS = 55;
...
export function renderQueue(state) {
  ...
  // An intent line costs the hero ~27px and a wrapped command ~60px, and a
  // stack row is 72px — so either one buys its space from the stack.
  var rows = '';
  var stacked = 0;
  if (!answering) {
    var wraps = String(summarize(heroAsk) || '').length > SUMMARY_COLS;
    var stackMax = (heroAsk.intent || wraps) ? STACK_MAX - 1 : STACK_MAX;
    var rest = [];
  ...
  if (answering) {
    ...
    // Counted from the rows that were actually laid, not the cap: whenever the
    // hero buys a row the footer would otherwise claim a card that isn't there.
    var shown = 1 + stacked;
